...
The published package makes a small set of promises to whoever installs it, and this capability is where they are stated: which Python interpreters are supported, that a plain install can already render and not only step, and that the metadata saying so lives in exactly one standards-based manifest. It also covers the repository side — a committed lock file so a contributor and CI resolve identical versions, and lint, format, type-check and test gates that run against the real package. The last requirement is the one with teeth: a job that skips its checks because the project does not match what it expects must show that, rather than reporting a pass it never earned.
